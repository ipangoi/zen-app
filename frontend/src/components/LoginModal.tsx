...
export default function LoginModal({ isOpen, onClose, onLogin, onChange }: LoginModalProps) {
    ...
    const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            onLogin(fetchedUsername);
            
            setEmail("");
            setPassword("");
        } catch (error: any) {
            let rawError = error.response?.data?.message || "";
            let cleanMessage =  `Connection to server failed! Please Try Again`;
            if (rawError.includes("failed on the 'email' tag")) {
                ...
                cleanMessage = "Password Too Short (Min 6)";
            } else {
